(* Original File can be found at
 ...
 TODO: Limit the number of reuests from server side.
 Also calculate average response time.
  *)
...
let num_requests = try int_of_string Sys.argv.(2) with _ -> 32

let pool = T.setup_pool ~num_domains:(num_domains - 1) ();;
let values = Array.make 1000 (Unified_interface.MVar.create_empty ());;
let result = Array.make 1000 (Unified_interface.MVar.create_empty ());;
let i = Atomic.make 0
...
let request_handler _ reqd =
  ...
  | "/" -> 
		...
		begin
		    Atomic.incr i;     
		    let _ = Unified_interface.MVar.put (values.(index)) (45) in ();
...
let () = 
    ...
    let pool = T.setup_pool ~num_domains:(num_domains - 1) () in
            let dindex = Atomic.make 0 in
            T.run pool (fun () -> 
                while true do
                    let domainResponse = (Atomic.get dindex) in
                    let eioRequest = (Atomic.get i) in
                    if domainResponse < eioRequest && domainResponse < num_requests then
                    begin
                        let promise = T.async pool (fun _ -> 
                        let index = Atomic.get dindex in
                        let n = Unified_interface.MVar.take (values.(index)) in
                        let ans = fib_par pool (n) in
                        let _ = Unified_interface.MVar.put (result.(index)) ans in

                        printf "\n%d Fib %d ans %d" (index) n ans;
                      ) in
                      T.await pool promise;
                      Atomic.incr dindex;             
                    end
                done;
            );
